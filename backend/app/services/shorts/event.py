"""event.py — ShortsCap backend: shorts event service.

ShortsEventService — business-level operations for Shorts events:
  ...
  * event_type / duration_seconds / metadata are validated (schema layer)

No Shorts detection happens here — Android submits the events.
"""
...
from app.repositories.shorts import ShortsEventRepository
...
class ShortsEventService:
    """Business operations for Shorts events."""

    def __init__(self, db: Session) -> None:
        self.db = db
        self.repository = ShortsEventRepository(db)
    ...
    def create_event(self, user_id: int, data: dict) -> ShortsEvent:
        """Validate and persist one Shorts event for the user."""
    ...
    def list_events(
        ...
        device or occurred-at range. Only the caller's own events are ever
        returned."""
        if start_date is not None and end_date is not None and start_date > end_date:
            raise ShortsValidationError("start_date must not be after end_date.")
