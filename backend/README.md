...
| `activity/ActivityRepository.kt` / `ActivityModels.kt` / `activity/PackageClassifier.kt` / `activity/AppUsageColorProvider.kt` | Real reporting aggregation over `shorts_usage` + `screen_activity_usage` (Phase 1: no seeds). Phase 1.2: system/IME/launcher/ShortsCap-internal packages are filtered at the reporting layer via `PackageClassifier` (totals, timeline, distribution, "Other" and Home all use the same reportable user-app set; raw `screen_activity_usage` rows and the Room schema are unchanged). Phase 1.3: selected-hour per-app breakdown (`hourApps`) + centralized deterministic app colors (`AppUsageColorProvider`). Phase 1.4: the device launcher/IME set resolved at app start is shared with the pure report path (`installDeviceNonReportablePackages`) so Activity and Home exclude the same packages. Phase 1.5: the Daily view is APPLICATION-centric — the donut/legend/detail aggregate `report.distribution` per application (consolidated across the day, system rows filtered before aggregation) instead of exposing hourly segments; the hourly timeline data remains internal (`aggregateByHour`/`splitByHour`/`hourApps`) for other features. Phase 1.6: the Daily dashboard is polished — real launcher icons per app (`ScAppUsageIcon`), deterministic non-gray fallback colors for unknown reportable apps (`AppUsageColorProvider`), no percentages in the Daily list (`ScTimeLegend.showPercent`), and the small Most Used Apps donut is duration-proportional (minutes, not percent) — all from the same `report.distribution` dataset. Phase 1.7: Android PackageManager is the source of app identity — a device-local `UserAppCatalog` (launchable apps minus launcher/IMEs/internals) is resolved at app start and is the single eligibility gate in `reportableSessions` (a recorded foreground package is reportable only if it is an installed user-facing app); display labels come from `loadLabel()` (`org.telegram.messenger` → "Telegram", never a stale backend `app_name`) — no device blacklists, raw rows and Room schema unchanged. Phase 1.8: Home Recent Activity is consolidated at the reporting layer (`recentActivityFromSessions` + `RecentActivityItem`) — duplicate `screen_activity_usage` sessions of one app become ONE row per package (summed duration, latest activity timestamp), applied BEFORE the Home top-3; raw rows, Room schema and all other reports unchanged | Unchanged; a future backend fills the same `ActivityReport` shape |
...
- **Phase 1.5 (Sep 4, 2026) — strict 24-hour cycle + corrupt-cycle reset (Android-side only):** a removed DEBUG `pause` helper had persisted `cycleExpiresAt ≈ now + 365 days` (original expiry kept only in memory), producing the observed `8489:34:56` countdown and a phantom year-long ACTIVE/LIMIT_REACHED cycle. Fixes in `ShortsControlEngine` / `ShortsLimitPageState` / `ShortsCapApplication`: `currentState()` now self-heals any ACTIVE row whose expiry exceeds `now + 24h` IN PLACE to CONFIGURED (limit + daily counters preserved, count/timestamps zeroed — the Shorts Limit page resets to READY_TO_ACTIVATE, Consumed 0) and startup runs `repairCorruptCycle()` before any consumer reads; the debug pause/resume footgun is deleted; `remainingCycleMillis` and the HH:MM:SS formatters are clamped to the 24:00:00 → 00:00:00 window so absurd values are impossible. A fresh ACTIVATE starts `now → now + 24h` with count 0. Platform Usage on the page continues to poll the real backend `platform_usage` aggregation. No Room schema change, no migration, no backend change; detection/counting/HUD/enforcement and historical usage data untouched. Tests: engine 36/36, page-state 29/29, store 5/5; full suite 291/302 (11 pre-existing `ShortsMonitoringPipelineTest` counting failures unchanged).
- **Phase 18.9 (Sep 6, 2026) — DEBUG-only enforcement simulation (Android-side only):** new `DebugEnforcementSimulation` + DEBUG-only `DebugEnforcementPanel` (Settings → Short Control, composed only when `BuildConfig.DEBUG`). The simulation keeps a strictly separate in-memory simulated cycle (its own `ShortsControlEngine` over an `InMemoryShortsLimitCycleStore` — no Room row, no `shorts_usage`/`shorts_events` write, real cycle untouched) and feeds simulated count + simulated short-form surface through the REAL production decision: engine state derivation + `shouldRestrict(surfaceActive, controlState)` (the exact boolean the `ShortsRestrictionEngine` overlay consumes). Real configured limit is read read-only as the sim default; non-positive/zero limits are rejected with the engine's own semantics. Enforcement trigger is edge-triggered exactly once per crossing (logcat tag `SC_DEBUG_ENFORCEMENT`: `SIM_START`/`SIM_INCREMENT`/`SIM_BATCH_INCREMENT`/`SIM_SET_COUNT`/`SIM_LIMIT`/`SIM_SURFACE`/`SIM_DECISION`/`SIM_ENFORCEMENT_TRIGGERED`/`SIM_ENFORCEMENT_COMPLETED`/`SIM_RESET`); surface leave lifts the simulated blocker, return re-triggers, staying blocked never duplicates. RESET clears only the simulation (count → 0); real count/limit/history/settings/monitoring are never reset or modified. Release safety is a double gate — every entry point short-circuits on `!BuildConfig.DEBUG` AND the panel is DEBUG-composed only. Production enforcement, engine, pipeline, HUD, restriction engine, Room schema and the backend are untouched. Tests: new `DebugEnforcementSimulationTest` 15/15; `:app:compileDebugKotlin` + `:app:assembleDebug` PASS. Limitation: the sim validates the real enforcement decision path, not the real full-screen overlay rendering end-to-end (that final device check still requires the real limit to be reached while watching on a short-form surface).
