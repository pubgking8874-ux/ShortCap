...
- **Circular widget centered:** each carousel page now centers its ring horizontally (`Modifier.align(Alignment.CenterHorizontally)`) — previously the ring was left-aligned inside the page — and the dark full-circle **track behind the ring was removed**; only the animated progress ring remains. Value and subtitle stay perfectly centered; swipe, animation, and page indicators unchanged.
