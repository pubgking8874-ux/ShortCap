...
- **Circular widget truly centered:** each pager page is wrapped in a full-width `Box(contentAlignment = Center)` so the ring, value, and subtitle sit in the exact middle of the card on every page (true center alignment — no reliance on pager alignment semantics or manual offsets). The dark gradient panel behind the widget was replaced with the standard card background for a clean, premium look, and the dark full-circle track behind the ring was removed earlier — only the animated progress ring remains. Swipe, animation, and page indicators are unchanged.
