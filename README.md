...
### Phase 18 — Real reporting data: Home / Activity / Recent Activity read persisted data *(Phase 1 data-consistency — Sep 4, 2026)*
**Files:** `activity/ActivityRepository.kt` (rewritten — real Room-backed aggregation, all seeds removed), `activity/ActivityModels.kt` (+`unlockCount`, `avgSessionSeconds`, `shortsByPlatform`, `PlatformShortsSlice`), `db/ShortsStoreDao.kt` + `db/ScreenActivityDao.kt` (+read-only range/recent queries — no schema change), `viewmodel/AppViewModel.kt` (single 5 s reporting poll; Home ring / Quick Stats / Recent Activity from real data; zero-state defaults instead of mocks), `screens/home/HomeScreen.kt` (hardcoded Recent Activity rows removed), `screens/activity/ActivityScreen.kt` + `screens/activity/ActivityReportScreen.kt` (real stats + per-platform Shorts breakdown, data-refresh tick), `ShortsCapApplication.kt` (installs the reporting data source), `app/src/test/java/com/shortscap/app/activity/ActivityRepositoryTest.kt` (new)

- **Shorts reporting** (`Home` ring, `Insights`, Activity report Shorts cards) now reads the persisted `shorts_usage` table — count = `SUM(countDelta)`, watch time = `SUM(durationMillis)`, platform breakdown = the stored `platform` values. The counting pipeline is untouched.
- **App-usage reporting** (Activity timeline, most-used-apps distribution, Unlock Count / Avg Session stats, Home Recent Activity, Apps Used Today) now reads the persisted `screen_activity_usage` sessions — the hardcoded Instagram/Chrome/WhatsApp rows and the seeded `ActivityRepository` dataset are gone.
- **One definition of "Today"** across every reporting screen: the device's **local calendar day** (`ZoneId.systemDefault()`, half-open `[todayStart, tomorrowStart)`). The 24-hour enforcement cycle (`ShortsControlEngine`) is untouched and is never used as "Today".
- **No mock flash** — `AppUiState.homeMetrics` defaults to zeros ("0m" / "0 Shorts") until the first real poll; the fake "1h 30m / 245 Shorts" defaults were removed.
- **Refresh** — a single 5 s reporting poll (replacing the old 1 s engine poll + 5 s usage poll) updates Home, bumps an `activityDataTick` so Activity/Reports recompute, and feeds Insights; no polling removed on other screens.
- **Tests** — new `ActivityRepositoryTest` covers the acceptance criteria (zeros for empty data, 5-Shorts aggregation + platform breakdown, real session distribution, today-boundary exclusion, weekly Monday-first window, daily trend); all existing suites unchanged. Monitoring/counting logic untouched.

### Phase 18.1 — Screen Activity session boundary + user-app reporting filter *(Phase 1.2 — Sep 4, 2026)*
**Files:** `activity/PackageClassifier.kt` (new — single conservative package classifier), `activity/ActivityRepository.kt` (filter sessions BEFORE aggregation: totals, timeline, distribution, "Other", unlock/avg), `viewmodel/AppViewModel.kt` (Home Apps Used Today + Recent Activity use the same classifier; filter before take-3), `screenactivity/ScreenActivityEngine.kt` (+`closeActiveSession()` — idempotent screen-off boundary), `accessibility/ShortsCapAccessibilityService.kt` (+`ACTION_SCREEN_OFF` receiver, registered on connect / unregistered on unbind+destroy), `app/src/test/java/com/shortscap/app/activity/PackageClassifierTest.kt` (new), `ActivityRepositoryTest.kt` (+7 filtering tests), `ScreenActivityEngineTest.kt` (+4 screen-off tests)

- **Problem A — locked/screen-off time inflated usage.** The collector only closed a session when a DIFFERENT package's window event arrived, so the last window (typically `com.android.systemui` keyguard) stayed "foreground" across the whole locked/idle period — observed as a single `systemui ≈ 13h 2m` session and a false `16h 55m` daily total. Fix: the accessibility service now registers an `ACTION_SCREEN_OFF` receiver that calls `ScreenActivityEngine.closeActiveSession()` the moment the screen turns off, persisting only the real foreground duration. The close is idempotent (a later service stop / repeated SCREEN_OFF closes nothing and never creates a duplicate row). No session is ever started on unlock or screen-on — a new session begins only when a real foreground window event arrives, so unlock time is charged to nobody.
- **Problem B — system components appeared as user apps.** `com.android.systemui`, the Gboard IME (`com.google.android.inputmethod.latin` → "latin"), the bare `android` system package and the launcher search surface were recorded as app sessions and shown in Activity/Home. Fix: the new `PackageClassifier` is the SINGLE source of truth for reporting eligibility and is used by Activity AND Home (never a second hardcoded list).
- **Classifier rules (conservative).** Categories: `USER_APP / SYSTEM_UI / SYSTEM_SERVICE / INPUT_METHOD / LAUNCHER / SHORTSCAP_INTERNAL / UNKNOWN`. Default: `UNKNOWN → reportable` (an unknown package is never hidden). No broad `com.android.*` ban and no `FLAG_SYSTEM`-only rule (Chrome/YouTube stay reportable). Explicit knowns: `com.android.systemui` → SYSTEM_UI; `android` → SYSTEM_SERVICE; IMEs (Gboard + a small common-IME baseline) → INPUT_METHOD; `com.google.android.googlequicksearchbox` → SYSTEM_SERVICE (documented Phase 1.2 decision: on the observed device this preinstalled package is recorded when the launcher's Google search widget / Discover surface is the active window, which is not meaningful standalone app usage — NOT a broad Google rule, other Google apps stay reportable); `com.shortscap.app` → SHORTSCAP_INTERNAL (recording continues; only reporting excludes it). A context-aware pass additionally recognizes the device's real launcher (`ACTION_MAIN` + `CATEGORY_HOME`) and its enabled input methods (`InputMethodManager`), cached and crash-safe.
- **Filtering happens BEFORE aggregation.** `ActivityRepository` drops non-reportable sessions once, before totals / timeline / distribution / percentages / "Other" / unlock-count / avg-session are computed, so `Other` can never become a hidden bucket of system UI. `AppViewModel` filters today's sessions before the distinct-app count, and filters a 50-row recent batch BEFORE taking the Recent Activity top-3 (system rows can no longer consume the slots).
- **Raw data preserved.** No `screen_activity_usage` row is deleted, no Room schema change, no migration, no backend/sync change — filtering is purely at the reporting layer.
- **Shorts untouched.** `ShortsMonitoringPipeline`, aggregators, adapters, `ShortsControlEngine`, HUD, `shorts_usage`/`shorts_events` and enforcement were not modified. 11 pre-existing `ShortsMonitoringPipelineTest` failures ("expected 1 but was 0" counting assertions — the known YouTube-adapter/counting state) remain untouched and are reported separately.
- **Tests & build.** New/updated tests: `PackageClassifierTest` (8), `ActivityRepositoryTest` (+7 filtering tests), `ScreenActivityEngineTest` (+4 screen-off tests). `:app:testDebugUnitTest` focused suites pass; full suite 265/276 pass (11 pre-existing Shorts pipeline failures above). `:app:assembleDebug` passes.
- **Runtime verification (device) — pending.** Install the debug build and follow the Phase 1.2 runtime checklist: lock the phone for several minutes with an app open and confirm the locked duration is not charged; open Activity and confirm systemui / latin / android / ShortsCap / launcher no longer appear; confirm the daily total reflects real user-app usage (not 16h55m); confirm Home Recent Activity shows real user apps; watch qualifying Shorts and confirm counting/HUD/enforcement are unchanged.

### Phase 18.2 — Activity donut details + Most Used Apps cleanup + consistent app colors *(Phase 1.3 — Sep 4, 2026)*
**Files:** `activity/AppUsageColorProvider.kt` (new — centralized deterministic app-color provider), `activity/PackageClassifier.kt` (+known launchers + bare/dotless-package rule), `activity/ActivityModels.kt` (+`ActivityAppUsage`; `ActivitySlice.packageName`), `activity/ActivityRepository.kt` (+`hourApps()` / `hourAppsFromSessions()`), `screens/activity/ActivityScreen.kt` (donut-center date removed, selected-hour Apps Used, colors via the provider), `charts/ChartRenderer.kt` (`ScPointTooltipCard` + optional Apps Used section), `theme/Color.kt` (+8 brand color constants), `viewmodel/AppViewModel.kt` (Home Recent Activity colors delegate to the provider), `app/src/test/java/com/shortscap/app/activity/PackageClassifierTest.kt` (+5), `ActivityRepositoryTest.kt` (+6), `AppUsageColorProviderTest.kt` (new, 6)

- **Donut center — total only.** The Daily donut center previously showed `1h 38m` + `Friday, September 4`. The date was redundant (the Usage Timeline card header already shows it above the donut), so the center now shows ONLY the total. The date remains in the Usage Timeline header and in the selected-hour detail card.
- **Selected hour shows real applications.** Tapping a Daily donut segment (or bar) still uses the existing detail card, which now adds an "Apps Used" list: real persisted `screen_activity_usage` sessions, filtered through the SAME `PackageClassifier`, split across hour boundaries with the SAME `splitByHour` logic as the timeline (a session crossing the hour contributes only its overlap — 12:55–1:10 PM gives 10m to the 1 PM hour, never 15m), grouped per package, sorted duration DESC. The breakdown sums to the same hour total as the timeline (normal rounding). Weekly/Monthly points carry no per-hour list.
- **Most Used Apps cleanup.** The suspicious entries were traced to their actual packages: `launcher3` = `com.android.launcher3` (the device's home app, confirmed in the device log) → classified `LAUNCHER` (static known-launcher list + the existing dynamic launcher detection); bare dotless package strings (`android`, `app`, `launcher3`) are not valid installed packages and are classified `SYSTEM_SERVICE` by a package-identity rule — never a display-label ban, so a real dotted package ending in `.app` stays reportable. Filtering still happens BEFORE aggregation, so `Other` can never hide system time.
- **Consistent application colors.** New centralized `AppUsageColorProvider` maps package identity → recognizable brand color (YouTube red, Instagram pink, Facebook blue, WhatsApp green, Chrome blue, Snapchat yellow, TikTok cyan, X, Telegram, LinkedIn, Reddit — new constants beside the existing Instagram/Chrome/WhatsApp ones). Deterministic: the same app has the same color regardless of duration, ranking, sort order or screen. Used by the Activity distribution donut + legend, the report-screen legend, the selected-hour app rows, and Home Recent Activity rows (AppViewModel delegates). `Other` and unknown packages stay neutral; the hourly timeline donut keeps its existing cycling palette (an hour is not a single app).
- **Scope.** No Room schema change, no raw data deleted, no Shorts/HUD/enforcement/backend/sync changes. The 11 pre-existing `ShortsMonitoringPipelineTest` failures remain untouched and reported separately.
- **Tests & build.** Focused Phase 1.3 suites 58/58; full suite 282/293 (11 pre-existing Shorts pipeline failures unchanged); `:app:assembleDebug` passes.
- **Runtime verification (device) — pending.** Install the debug build and verify: donut center shows only the total; tapping an hour shows the real per-app breakdown; `android`/`app`/`launcher3`/systemui do not appear in Most Used Apps; YouTube/Instagram keep their brand colors everywhere and colors are stable across refreshes; Home Apps Used Today / Recent Activity still filter correctly; Shorts counting/HUD/enforcement unchanged.

### Phase 18.3 — Consolidated app usage + device-package parity *(Phase 1.4 — Sep 4, 2026)*
**Files:** `activity/PackageClassifier.kt` (+`resolveDeviceNonReportablePackages()`), `activity/ActivityRepository.kt` (+`installDeviceNonReportablePackages()`; `reportableSessions` also excludes the device set), `ShortsCapApplication.kt` (resolves + installs the device launcher/IME set at app start), `app/src/test/java/com/shortscap/app/activity/ActivityRepositoryTest.kt` (+3 consolidation / device-parity tests)

- **Audit outcome.** Most Used Apps already groups by package and sums duration (one consolidated row per app), and the Daily donut keeps its truthful TIME semantics (each segment = one hour); app identification lives in the existing detail card below the chart (single-app hours show that app; multi-app hours list every app with its hour-total duration — never repeated session timestamps, never a false single-app assignment). The named leak packages were already excluded in Phase 1.2/1.3 by package identity: `android` → SYSTEM_SERVICE, `com.android.launcher3` → LAUNCHER, bare `app`/`launcher3` → SYSTEM_SERVICE (dotless, not a valid installed package), systemui → SYSTEM_UI, IMEs → INPUT_METHOD, ShortsCap → SHORTSCAP_INTERNAL.
- **Remaining gap closed.** Activity's pure aggregation path (no Context) previously knew only the STATIC classifier rules, while Home used the context-aware classifier — so a device-specific launcher or IME NOT in the static lists could still reach Activity while Home hid it. Phase 1.4 closes that parity gap: `PackageClassifier.resolveDeviceNonReportablePackages(context)` resolves the device's real launcher + every enabled input method ONCE at app start (`ShortsCapApplication`), and `ActivityRepository.installDeviceNonReportablePackages()` lets the pure report path exclude exactly the same packages. Package identity only — never display labels, no per-screen lists.
- **Consolidation verified by tests.** Same app in several sessions across the day → ONE Most Used Apps row with the summed duration (YouTube 20m + 15m + 30m = one 65m row); the same app twice inside one hour merges into a single selected-hour row (YouTube 30m + Instagram 15m from three sessions) — no repeated timing entries anywhere.
- **Scope.** No Shorts monitoring/detection/counting/enforcement/HUD/adapters, no Room schema change, no raw data deleted, no backend/sync/auth changes.
- **Tests & build.** Focused Activity + classifier suites green (PackageClassifierTest 13, AppUsageColorProviderTest 6, ActivityRepositoryTest 22, ScreenActivity suites green); full suite 285/296 (11 pre-existing ShortsMonitoringPipelineTest failures unchanged); `:app:assembleDebug` passes.
- **Runtime verification (device) — pending.** Confirm a device-specific launcher/IME never appears in Activity while Home already hides it, and Most Used Apps shows one consolidated row per app.

### Phase 18.4 — Shorts Limit: strict 24-hour cycle + corrupt-cycle reset *(Phase 1.5 — Sep 4, 2026)*
**Files:** `shorts/ShortsControlEngine.kt` (self-healing repair in `currentState()`; `remainingCycleMillis` capped at one 24h window; removed the debug `pause`/`resume` footgun that persisted ~1-year expiries), `shorts/ShortsLimitPageState.kt` (`remainingCountdownHms` / `remainingHoursMinutes` clamp to 24:00:00 max), `ShortsCapApplication.kt` (runs `repairCorruptCycle()` at startup, before any consumer reads the cycle), `app/src/test/java/com/shortscap/app/shorts/ShortsControlEngineTest.kt` (+4: corrupt self-heal, healthy untouched, activation-after-repair, 24h window bounds), `ShortsLimitPageStateTest.kt` (+2: formatter never exceeds 24:00:00 — the historical `8489:34:56` input formats as `24:00:00`)

- **Root cause of the `8489:34:56` countdown.** The engine's own expiry math was timestamp-correct (`remaining = cycleExpiresAt − now`); the corrupt value came from a DEBUG-only `debugPauseCycle()` helper that **persisted** `cycleExpiresAt = now + ~365 days` while keeping the true expiry only in an in-memory field. After any process death/restart the original expiry was unrecoverable, so the persisted row showed ~354 days (~8489 hours) remaining, `Consumed 397`, and the cycle stayed ACTIVE/LIMIT_REACHED for a year.
- **Fix 1 — corrupt-cycle self-heal (the reset).** `currentState()` now detects any ACTIVE row whose expiry is more than one full window in the future (`cycleExpiresAt > now + 24h` — impossible for a real cycle, since expiry = start + 24h and start ≤ now) and converts it IN PLACE to `CONFIGURED` on every read: the user's saved limit and the daily monitoring counters are **preserved**, the phantom count/timestamps are zeroed. The Shorts Limit page then shows READY_TO_ACTIVATE (no countdown, Consumed 0, Remaining = limit) and a fresh ACTIVATE starts an exact 24-hour cycle from zero. `ShortsCapApplication` also calls `repairCorruptCycle()` at startup so the heal happens before the HUD / restriction engine / page can read the stale row. Idempotent — a healthy cycle is never touched (verified by test).
- **Fix 2 — debug pause removed.** `debugPauseCycle()` / `debugResumeCycle()` / `isDebugPaused()` and the in-memory expiry field are deleted (they had zero production or test callers), so the corruption source can never recreate a >24h persisted expiry. `debugResetCycle` / `debugSetCount` / `debugSetLimit` / `debugClearLimitReached` (all proper 24h-consistent, DEBUG-gated) remain.
- **Fix 3 — countdown can never exceed 24:00:00.** `deriveState()` caps `remainingCycleMillis` to `[0, 24h]`, and the display helpers `remainingCountdownHms()` / `remainingHoursMinutes()` additionally clamp their input to the same window — even a stale/absurd duration can never render as `25:00:00`, `100:00:00` or `8489:34:56`. The valid range is strictly `24:00:00 → 00:00:00`.
- **Cycle model.** ACTIVATE → `cycleStartedAt = now`, `cycleExpiresAt = now + 24h` (single authoritative `CYCLE_DURATION_MILLIS` constant), `currentCount = 0`. Remaining is always `cycleExpiresAt − currentTime` (timestamp-derived, reconstructed after restart — never a persisted decreasing counter). Expiry at `now ≥ cycleEnd` marks EXPIRED (no auto-roll unless the existing `autoRestartEnabled` rule applies, and only at the exact expiry boundary). One authoritative active cycle — ACTIVATE can never duplicate or overlap a running cycle.
- **Platform Usage — unchanged, real data.** The page's per-platform rows continue to come from the existing authoritative pipeline: the app's counted `shorts_usage` syncs to the backend, and the screen polls the backend `platform_usage` aggregation every 15 s while on-screen (`ShortsControlSyncer.fetchControl()` → `ShortControlResponse.platformUsage`). No local counting was added, nothing is fabricated, and the honest "No Shorts usage recorded yet." empty state is preserved. Cycle-window scoping of that backend aggregate is a backend-contract concern and stays out of this Android phase.
- **Scope.** Shorts detection/counting/qualification/state machine, adapters, HUD, enforcement, `RoomShortsLimitCycleStore`, Room schema, migrations, Activity reporting, auth and backend/sync were NOT modified. Historical `shorts_usage` / reporting data is untouched — only a stale enforcement-cycle row is healed (never deleted).
- **Tests & build.** Focused: `ShortsControlEngineTest` 36/36, `ShortsLimitPageStateTest` 29/29, `RoomShortsLimitCycleStoreTest` 5/5. Full suite 291/302 — the 11 pre-existing `ShortsMonitoringPipelineTest` failures ("expected 1 but was 0") are unchanged and remain out of scope. `:app:assembleDebug` passes.
- **Runtime verification (device) — pending.** Install the debug build and check: the Shorts Limit page no longer shows `8489:34:56` (it shows the READY state / no countdown and Consumed 0 after the startup repair); activating 390 shows ≈24:00:00 / Consumed 0 / Remaining 390 / 0%; watching real Shorts raises Consumed from the authoritative counter and populates Platform Usage; leaving the page or restarting the app keeps the same cycle timestamps; no overlapping cycles can be created.

### Phase 18.5 — Activity Daily is application-centric: app donut + app selection, no time-of-day *(Phase 1.5 — Sep 4, 2026)*
**Files:** `screens/activity/ActivityScreen.kt` (DAILY now renders the per-application donut from `report.distribution` — the SAME reportable dataset as Most Used Apps — with app selection/highlighting and an app detail card; barred/line/graph styles render the same app data via `ScDistributionChart`; Weekly/Monthly keep their existing time-series views), `charts/ChartRenderer.kt` (`ScTimeLegend` +`selectedIndex` row highlight; `ScPointTooltipCard` +optional `titleDotColor`), `app/src/test/java/com/shortscap/app/activity/ActivityRepositoryTest.kt` (+`daily app donut dataset is application based not hourly`), `PackageClassifierTest.kt` (+Messenger/Telegram stay reportable — audit result)

- **Root cause of the hourly UI.** The Daily tab had always been a TIME-series view: `ActivityRepository` build DAILY points as the 24-hour timeline and `ActivityScreen` fed those points straight into the donut + legend (`12 AM`, `7 AM`, …) with a tooltip showing `detailTitle` (date) + `timeRange` ("1:00 PM – 2:00 PM"). The per-application dataset (top apps consolidated by package, system filtered) already existed as `report.distribution` — it powered the Most Used Apps card but was NOT the Daily donut.
- **Daily is now application-centric.** The Daily Usage Timeline card answers "which applications did I use today, and for how long?": the donut segments ARE applications (YouTube / Instagram / …), each colored through the existing `AppUsageColorProvider`; the center shows ONLY the total; the legend lists the same apps with exact durations. The dataset is literally the same `report.distribution` as Most Used Apps — one source of truth, so the donut and the list always agree (65m YouTube in the list = 65m YouTube segment).
- **Donut click → application selection.** Tapping a segment (or a legend row, or a Most Used Apps row) selects that application by PACKAGE identity (`selectedAppPackage`): the segment highlights (wider arc, others dimmed), the legend row and the matching Most Used Apps row highlight too, and a detail card shows the app name + its total daily usage (color dot included) — with NO date, NO "Time: 1:00 PM – 2:00 PM", NO session timestamps. "Other" has no package, so tapping it just deselects. Weekly/Monthly keep their existing day/date-range tooltips (they never carried hour-of-day data).
- **`android` / `app` / `Messenger` investigation (package identity, not display labels).** `android` (bare) → classified `SYSTEM_SERVICE` and bare dotless `app` → `SYSTEM_SERVICE` since Phase 1.2/1.3; both are excluded before aggregation and covered by tests — the screenshot showing them predates those fixes. The displayed "Messenger" is either the genuine Messenger package `com.facebook.orca` or Telegram `org.telegram.messenger` — both LEGITIMATE user apps; the collector records only real `TYPE_WINDOW_STATE_CHANGED` foreground windows and `appName` is only ever populated from backend sync (`app_name`), so without evidence of a collection bug these must stay visible (classifier test added documenting this). If a Messenger session exists in `screen_activity_usage`, it is real recorded foreground data for the selected day.
- **Hourly data preserved internally.** `aggregateByHour`, `splitByHour`, `hourApps` and the 24-point DAILY report remain in the repository (other features may use them); only the Daily user interaction no longer exposes hour-of-day.
- **Scope.** No Room schema change, no raw `screen_activity_usage` rows deleted, no Shorts monitoring/control/HUD/backend/sync changes; `AppUsageColorProvider` remains the single color source.
- **Tests & build.** New: `ActivityRepositoryTest` "daily app donut dataset is application based not hourly" (per-app consolidation + system filtered + distribution != 24-hour points), `PackageClassifierTest` "messenger is a legitimate user app and stays reportable". Main source compiles (`:app:compileDebugKotlin` PASS); focused/full suites and `assembleDebug` to be re-run with the device build (pre-existing `ShortsMonitoringPipelineTest` failures unchanged).
- **Runtime verification (device) — pending.** Open Activity → Daily: the donut shows applications (no `12 AM`/`7 AM`/`1 PM – 2 PM` anywhere); tapping YouTube highlights its segment + rows and shows "YouTube / Usage: 65m"; Most Used Apps and the donut match exactly; switching to Week/Month still works; system packages stay out.

### Phase 18.6 — Activity polished app-usage dashboard: real icons, prominent colors, no percentages *(Phase 1.6 — Sep 4, 2026)*
**Files:** `activity/AppUsageColorProvider.kt` (deterministic fallback palette for unknown reportable apps — no more gray), `components/EntityIcon.kt` (+`ScAppUsageIcon` — real launcher icon by package, brand-letter tile fallback), `charts/ChartRenderer.kt` (`ScTimeLegend` +`showPercent` + per-slice `icon` slot; `ScPointTooltipCard` +`titleIcon`), `screens/activity/ActivityScreen.kt` (app icons in the Daily legend + Most Used Apps rows + selected-app card; Daily list no percentages; small Most Used Apps donut is now duration-proportional; stale doc updated), `app/src/test/java/com/shortscap/app/activity/AppUsageColorProviderTest.kt` (unknown apps are deterministic non-gray; `Other` stays neutral), `ActivityRepositoryTest.kt` (+`android and bare app sessions never reach the daily app dataset`)

- **Why the donut was mostly gray.** `AppUsageColorProvider` returned the neutral `PieOther` gray for ANY package not in the ~13 known brand map — so legitimate reportable apps (Messenger `com.facebook.orca`, region apps, games…) AND the "Other" slice all rendered gray. Fix (single source kept): unknown reportable packages now get a **deterministic palette color** keyed by package identity (hash → one of 8 non-gray hues; same app = same color on every screen/refresh/order), while only `Other` (null) stays neutral gray — gray is now purely the legitimate remainder/background.
- **Percentages removed from the Daily list.** `ScTimeLegend` gained `showPercent` (default on for Weekly/Monthly time legends) — the Daily application list passes `false`, so it shows ONLY the real app icon + name + total duration. No `%` anywhere in the Daily app-centric interaction.
- **Generic dots replaced by real app icons.** New `ScAppUsageIcon(packageName, name, …)` resolves the actual installed launcher icon via `PackageManager` (async, crash-safe); fallback is a brand-letter tile coloured by the SAME `AppUsageColorProvider` colour — never a fake icon, never gray for an app. Used in the Daily donut legend, the Most Used Apps rows and the selected-application detail card (via `ScPointTooltipCard.titleIcon`).
- **Small Most Used Apps donut is proportional.** Its `ChartSlice` values changed from `percent` to real `minutes`, so segment sizes reflect actual usage exactly like the main Daily donut; it still uses the same `report.distribution` (top-5 + `Other`, system filtered) and the same `pieColor` colours — one dataset, one colour source, donut ↔ list always agree (65m YouTube in the list = 65m segment in both rings).
- **`android` / `app` — verified at the data layer.** Both are classified `SYSTEM_SERVICE` (bare/dotless system artifact rule, Phase 1.2/1.3) and `reportableSessions()` filters them BEFORE aggregation; a new repository test feeds raw `android` + `app` sessions (1h each) into `buildReport` and asserts they never reach `distribution` nor the daily total. No display-name blacklist, no UI filter.
- **`Messenger` — real app, kept.** Its package (`com.facebook.orca` or Telegram `org.telegram.messenger`) is legitimate; the collector records only real foreground windows. It now gets a deterministic non-gray colour (not in the brand map) instead of hiding it.
- **Scope.** No Room schema change, no raw `screen_activity_usage` rows deleted, no Shorts monitoring/control/HUD/backend/sync changes; `AppUsageColorProvider` remains the single color source; Home unchanged.
- **Tests & build.** `ActivityRepositoryTest` 24/24, `PackageClassifierTest` 14/14, `AppUsageColorProviderTest` 7/7; `:app:assembleDebug` PASS; full suite 295/306 — the 11 pre-existing `ShortsMonitoringPipelineTest` counting failures are unchanged and out of scope.
- **Runtime verification (device) — pending.** Open Activity → Daily: donut segments are colourful and size-proportional (no gray apps, no percentages); every list row shows the real app icon; `android`/`app`/launcher/systemui/IME/ShortsCap absent; tapping YouTube highlights its segment + icon row and shows icon + name + total usage only; Main donut, legend, Most Used Apps and selected detail all match; Weekly/Monthly unchanged; Shorts counting/HUD/enforcement unchanged.

### Phase 18.7 — Universal user-app based Activity reporting: device catalog is the single gate *(Phase 1.7 — Sep 4, 2026)*
**Files:** `activity/UserAppCatalog.kt` (new — device-local user-facing application catalog resolved from Android PackageManager metadata: launchable apps minus device launcher / enabled IMEs / known internals, plus authoritative labels), `activity/ActivityRepository.kt` (+`installUserAppCatalog`; `reportableSessions` now gates on catalog membership; `ActivityAppNames` +`installDeviceLabels` and label priority — Android label first), `ShortsCapApplication.kt` (resolves + installs the catalog and labels at app start), `app/src/test/java/com/shortscap/app/activity/ActivityRepositoryTest.kt` (+4 focused Phase 1.7 tests)

- **Architecture — Android is the source of app identity.** New `UserAppCatalogResolver.resolve(context)` builds the catalog ONCE at app start from `PackageManager`: `queryIntentActivities(ACTION_MAIN + CATEGORY_LAUNCHER)` = installed + launchable + user-facing apps (System UI, launchers, keyboards/IMEs, services and background components register NO launcher activity, so they never enter); the device `CATEGORY_HOME` launcher and enabled IMEs are explicitly removed; the conservative `PackageClassifier` still drops known internals that ARE launchable (ShortsCap, quick-search surface). Result: a DEVICE-INDEPENDENT `UserAppCatalog(packages, labels)` — no manufacturer/OEM blacklist.
- **Catalog membership is the single reporting gate.** `ActivityRepository.reportableSessions()` now requires: package ∈ installed catalog AND not a device non-reportable package AND classifier-reportable — all BEFORE aggregation (totals, donut, legend, Most Used Apps, `Other`, unlock/avg). A recorded foreground package that is not an installed user-facing app is IGNORED for Activity reporting. Raw `screen_activity_usage` rows, monitoring, schema and collection are untouched; the gate is null by default (tests / pre-install) and installed at app start.
- **Telegram identity fixed at the data layer (no rename hack).** Display labels are now resolved from Android (`PackageManager.loadLabel`) at app start and stored as `UserAppCatalog.labels`; `ActivityAppNames.friendlyName` prioritises the device label over any persisted/backend `appName`. So `org.telegram.messenger` displays as **Telegram** (Android's own label) even when a synced `app_name` says "Messenger" — same principle for every installed app, and Home Recent Activity (which shares `friendlyName`) is fixed too. No `Messenger → Telegram` hardcode.
- **Icons** — already device-derived (`ScAppUsageIcon` → `PackageManager.loadIcon`, Phase 1.6); unchanged. Colors — `AppUsageColorProvider` remains the single source; unknown-but-real apps keep deterministic non-gray colors.
- **Consolidation unchanged.** Sessions of one package still merge into ONE daily total (YouTube 20m+30m+10m = 60m), no time-of-day; the Daily donut stays application-centric with duration-proportional segments (no forced 50%).
- **Scope.** No Shorts monitoring/control/HUD/adapters, no Room schema, no raw-data changes, no backend/sync changes.
- **Focused automated verification (per Phase 1.7 spec — no full suite, no manual device work):** `compileDebugKotlin` PASS; `ActivityRepositoryTest` 28/28 (new: catalog gate — uninstalled/background packages ignored; classifier-only restore; device label wins over persisted appName — Telegram; known-map fallback), `PackageClassifierTest` 14/14, `AppUsageColorProviderTest` 7/7.
- **Manual verification (user/device, out of scope for the coding task):** confirm the Daily/Activity lists show only real installed apps with their real Android labels/icons; Telegram shows "Telegram"; `android`/`app`/launcher/systemui/IME/ShortsCap absent; donut/list/selection consistent; Shorts unchanged.

### Phase 18.8 — Home Recent Activity: one row per application (consolidated duplicate sessions) *(Phase 1.8 — Sep 5, 2026)*
**Files:** `activity/ActivityRepository.kt` (+`recentActivityFromSessions()` — pure per-package consolidation), `activity/ActivityModels.kt` (+`RecentActivityItem`), `viewmodel/AppViewModel.kt` (Recent Activity poll now consolidates by package BEFORE the top-3 take; `toRecentEntity` maps the consolidated item), `app/src/test/java/com/shortscap/app/activity/ActivityRepositoryTest.kt` (+6 focused consolidation tests)

- **Problem.** Home → Recent Activity could show the same application multiple times (e.g. three YouTube rows — `11:43 PM 0m`, `11:43 PM 0m`, `11:31 PM 5m`) because it mapped each persisted `screen_activity_usage` foreground-session row directly to a UI row.
- **Fix — consolidate at the reporting layer.** The poll now runs the reportable-package filter first (unchanged — same `PackageClassifier` rules as before), then passes the filtered sessions to the new pure `ActivityRepository.recentActivityFromSessions()`, and only THEN applies the existing top-3. The aggregation groups by the stable Android `packageName` (never a display label): one item per application, `totalDurationSeconds` = the SUM of every session's duration, and `latestOccurredAt` = the most recent session's timestamp (rows are sorted newest-first by that latest activity).
- **Result.** Three YouTube sessions (`5m` + two sub-minute `0m`) render as ONE row — `YouTube — 11:43 PM — 5m` (summed real duration, latest activity time). The 3-row limit is applied AFTER consolidation, so raw duplicates can never hide another application (YouTube/YouTube/Chrome/YouTube/Telegram → YouTube, Chrome, Telegram).
- **Zero-minute records.** Tiny sessions still convert to `0m` naturally (no fabricated minimum), but they never appear as separate duplicate rows — they fold into their application's single consolidated duration.
- **Scope.** Raw `screen_activity_usage` rows untouched (no delete/merge/schema change); monitoring/session collection, Shorts pipeline, counting, HUD, limit cycle and every other reporting screen (Activity timeline/donut, Most Used Apps, selected-hour apps) unchanged. `HomeScreen.kt` / Recent Activity UI untouched — same card, icons, colors and one-row-per-app layout; only the underlying data is consolidated.
- **Focused automated verification (per Phase 1.8 spec):** `ActivityRepositoryTest` consolidation tests — duplicate sessions → one row; grouped per package (summed durations); latest timestamp selected + newest-first sort; consolidation before top-3; sub-minute `0m` sessions fold into one row; empty input → no rows. `:app:compileDebugKotlin` PASS.
- **Runtime verification (user/device, out of scope for the coding task):** open Home → Recent Activity and confirm each application appears at most once with its consolidated duration and latest activity time.

### Phase 18.9 — DEBUG-only enforcement simulation: test Shorts enforcement instantly *(Sep 6, 2026)*
**Files:** `shorts/DebugEnforcementSimulation.kt` (**new** — in-memory simulation + real-decision bridge), `screens/settings/DebugEnforcementPanel.kt` (**new** — DEBUG-only control surface), `screens/settings/ShortsControlScreen.kt` (renders the panel only under `BuildConfig.DEBUG`), `app/src/test/java/com/shortscap/app/shorts/DebugEnforcementSimulationTest.kt` (**new** — 15 focused bridge tests)

- **Problem.** Verifying the existing Shorts enforcement engine required watching 10/15/50/100 real Shorts or waiting out a real 24-hour limit cycle — nothing in the app let a developer exercise the enforcement decision instantly.
- **Debug-only simulation.** A new `DebugEnforcementSimulation` keeps a strictly separate, fully in-memory simulated cycle and feeds it through the REAL enforcement path — there is no second enforcement engine:
  - It owns its own `ShortsControlEngine` bound to an `InMemoryShortsLimitCycleStore`, so every count / limit / status / LIMIT_REACHED derivation runs the same engine class production uses. **No Room row, no `shorts_usage` / `shorts_events` write, no real-cycle mutation — nothing is ever persisted.**
  - A simulated "short-form surface is on screen" flag is fed into the SAME pure decision function the production enforcement layer consumes — `shouldRestrict(surfaceActive, controlState)` (the exact boolean `ShortsRestrictionEngine` uses to show/hide the real touch-blocking overlay). When that production decision turns true, the simulation fires the enforcement-trigger edge (logged `SIM_ENFORCEMENT_TRIGGERED` / `SIM_ENFORCEMENT_COMPLETED`) exactly once, edge-triggered like production: leaving the surface lifts the simulated blocker and returning re-triggers; staying blocked never duplicates.
  - The REAL configured Shorts limit is read (read-only) as the simulation's default limit — the sim never becomes a second, independent production limit. Zero / non-positive limits are rejected exactly as the engine rejects them (no fabricated zero-limit behavior).
  - **Release safety (double gate):** every simulation entry point short-circuits on `!BuildConfig.DEBUG` (an execution gate, not a hidden button), and the control panel is composed only when `BuildConfig.DEBUG`. A release/production user can never start the simulation, fake a count, alter the real count/limit or trigger enforcement through it.
- **Control surface.** Settings → Short Control shows an "ENFORCEMENT DEBUG (DEBUG BUILD ONLY)" panel in debug builds only: real limit (live read) + simulated count/limit + trigger counter, a decision line (NOT ENFORCED / WARNING / RESTRICT with surface state), and controls — START SIM (limit pre-filled from your real limit), +1 SHORT, +5 SHORTS, SET COUNT, SURFACE ON/OFF, RESET (clears only the simulation), STOP. All logs use the dedicated `SC_DEBUG_ENFORCEMENT` tag (`SIM_START`, `SIM_INCREMENT`, `SIM_BATCH_INCREMENT`, `SIM_SET_COUNT`, `SIM_LIMIT`, `SIM_SURFACE`, `SIM_DECISION`, `SIM_ENFORCEMENT_TRIGGERED`, `SIM_ENFORCEMENT_COMPLETED`, `SIM_RESET`).
- **Reset semantics.** `RESET` returns only the simulation to count 0 (surface off, blocker lifted); the real Shorts count, real limit, persisted usage history, monitoring, settings and the 24-hour cycle are never reset, deleted or modified by any method here.
- **Scope.** Not changed: `ShortsControlEngine` production behavior, `ShortsMonitoringPipeline` detection/qualification/counting, HUD (`ShortsHudController` / `ShortsHudTriggerEngine` / overlay), `ShortsRestrictionEngine` + restriction overlay, adapters, Room schema, the Shorts Limit settings UI, Activity reporting, backend. The debug panel is the only UI addition and it is DEBUG-only.
- **Verification.** `:app:compileDebugKotlin` PASS; `DebugEnforcementSimulationTest` 15/15 PASS (inert-by-default, start reads the real limit, below/at/above limit decisions, increment-by-1 / batch-by-N, surface gating, single-trigger on the crossing transition, no duplicate triggers while blocked, surface leave/return re-trigger, reset clears only the simulation, real count/limit untouched, non-positive and zero limit rejection, small-limit 0→1→2 walk, in-sim limit edits, stop → inert); `:app:assembleDebug` PASS.
- **Limitation (documented honestly).** The simulation exercises the real enforcement DECISION (engine derivation + `shouldRestrict`) and records the enforcement-trigger edge; it does not itself paint the real full-screen blocking overlay over a restricted app, because that overlay is driven by the real Room-backed cycle plus live accessibility surface events. End-to-end device validation of the overlay remains a separate manual step (below).
- **Manual device validation steps (developer, later):** (1) build/install the debug APK, Settings → Short Control → set a real Shorts limit and ACTIVATE it; (2) open Settings → Short Control and scroll to ENFORCEMENT DEBUG; (3) tap START SIM (limit pre-filled from your real limit), then +1 repeatedly and toggle SURFACE ON at the limit — watch logcat tag `SC_DEBUG_ENFORCEMENT` for `SIM_INCREMENT → SIM_DECISION decision=LIMIT_REACHED restrict=true → SIM_ENFORCEMENT_TRIGGERED → SIM_ENFORCEMENT_COMPLETED` firing exactly once; (4) continue +1 past the limit and confirm no duplicate `SIM_ENFORCEMENT_TRIGGERED`; (5) toggle SURFACE OFF/ON and confirm `SIM_ENFORCEMENT_LIFTED` then a second trigger; (6) RESET and confirm the simulated count returns to 0 while the Shorts Limit page's real Consumed count is unchanged; (7) to validate the real blocking overlay end-to-end, keep the real limit active, watch real Shorts until the real count reaches the limit on a short-form surface, and confirm the real full-screen restriction overlay appears (this still requires real watching — the sim is a decision-path test tool, not a substitute for that final device check).
...
- Monitoring statistics were removed from `MonitoringSettings` — Home's Quick Status derives **Today Usage** from the real persisted `shorts_usage` table (local calendar day), Activity's timeline/distribution come from `screen_activity_usage`, and the Web counts come from the Web rule list.
...
- The report shows the period chart in the selected chart style, plus a summary derived from the same structured data: total usage, busiest day, most-used app, trend %, and Shorts usage/count (aggregated from real persisted `shorts_usage` + `screen_activity_usage`) — all from `ActivityRepository.reportFor(period)`, nothing hardcoded.
...
- `activity/ActivityRepository.kt` — real persisted-data aggregation (`shorts_usage` + `screen_activity_usage`) + future backend seam.
...
- One real persisted source (app-usage sessions from `screen_activity_usage`, counted Shorts from `shorts_usage`) is aggregated dynamically per period — Daily groups app-usage sessions by hour (split across hour boundaries), Weekly by weekday, Monthly by 7-day date ranges. There are NO seeded datasets.
- "Today" is always the device's local calendar day — never the 24-hour enforcement cycle.
- Data stays entirely in the data layer; the charts only visualize. A future backend fills the exact same `ActivityReport` shape with no UI redesign.
...
- One real persisted record set (app-usage sessions from `screen_activity_usage`, counted Shorts from `shorts_usage`) is aggregated dynamically per period — Daily groups by hour (sessions split across hour boundaries), Weekly by weekday (Monday-first), Monthly by 7-day date range. `rangeReportFor(range)` drills into one range's per-day usage. No seeded datasets.
...
### Phase 4A — Controlled enforcement test harness: fast 5-Short loop against the REAL engine *(Sep 6, 2026)*

**Files changed:** `shorts/ShortsEnforcementTestHarness.kt` (**new** — DEBUG-only test harness), `shorts/ShortsRestrictionEngine.kt` (+ `debugTriggerEnforcement()` DEBUG-only seam), `screens/settings/DebugEnforcementPanel.kt` (+ REAL ENFORCEMENT TEST section), and this `README.md`.

- **Problem solved:** production minimum limit is 50 and the 24-hour cycle cannot be shortened in production, so validating the enforcement overlay required either watching 50+ real Shorts or mutating the real cycle via DEBUG controls. This phase adds a **strictly separate, in-memory, DEBUG-only test path** with a developer-set test limit (default 5) — no 24-hour wait, ever.
- **Real events, not fake counts:** the harness subscribes to the EXISTING `ShortsMonitoringPipeline.CountChangeListener` — the same notification the production enforcement seam receives after every real `countShort()` → `onShortCounted()` → Room persistence. A test event IS a fully validated production Short event (detection, 3s qualification, dedup unchanged). Invalid events can never inflate the test counter.
- **Real enforcement, not simulated:** when the test counter reaches the test limit, the harness calls the new `ShortsRestrictionEngine.debugTriggerEnforcement()` DEBUG seam, which runs the EXACT production evaluation (`ShortsControlEngine.shared.currentState()` → `shouldRestrict` → `ShortsRestrictionOverlayManager.show/hide`) and returns a diagnostic result (`SHOWN` / `SHOW_FAILED` / `DECISION_FALSE` / `NO_CONTEXT`) — the REAL full-screen touch-blocking overlay appears.
- **Production isolation:** the test counter/limit-live state lives ONLY in the harness (in-memory, `BuildConfig.DEBUG`-gated at every entry point and in the UI). The production `ShortsLimitCycle` (Room), daily monitoring count, daily reset, 24-hour circle, real limit, history, settings and auth are never read-for-write, never modified, never reset. `RESET TEST` clears ONLY harness state (count → 0, enforcement edge cleared) — the same test is immediately runnable again.
- **Logging:** dedicated `SHORTS_TEST` tag: `TEST_START` / `TEST_SHORT_ACCEPTED count=N limit=L` / `TEST_LIMIT_REACHED` / `TEST_ENFORCEMENT_TRIGGERED` / `TEST_ENFORCEMENT_EVAL` / `TEST_ENFORCEMENT_ACTION show success=` / `TEST_ENFORCEMENT_RESULT` / `TEST_ENFORCEMENT_BLOCKED reason=` / `TEST_RESET` / `TEST_COUNT=0` / `TEST_STOP`. All existing production logs preserved.
- **Release safety:** double gate — every harness/engine-seam entry point short-circuits on `!BuildConfig.DEBUG` AND the panel section is DEBUG-composed only. Release builds are fully inert.

### Phase 4A.1 — Controlled test enforcement: independent test counter, real-enforcement seam, HUD test indicator *(Sep 6, 2026)*

**Files changed:** `shorts/ShortsEnforcementTestHarness.kt` (rewritten — independent DEBUG test counter + HUD publishing), `shorts/ShortsRestrictionEngine.kt` (`debugTriggerEnforcement(testCondition)` seam now accepts the TEST decision input), `hud/ShortsHudContent.kt` (+ DEBUG test fields on `ShortsHudUiState` + "TEST MODE · n / limit" indicator inside the existing HUD chip), `screens/settings/DebugEnforcementPanel.kt` (updated harness stats/description), and this `README.md`.

- **Root cause fixed (from the Phase 4A audit):** the 4A harness mirrored the REAL engine count and its seam evaluated the production `limitReached` — so a 5-Short test could never fire the real overlay without mutating production state. 4A.1 corrects the architecture: the harness now owns an **independent in-memory DEBUG test counter** (`testCount++` per validated production Short event) and the DEBUG seam accepts the **test enforcement condition** while executing the **real enforcement action**.
- **Independent test counter:** the harness subscribes to the EXISTING `ShortsMonitoringPipeline.CountChangeListener` (fires only after real `countShort()` → `onShortCounted()` → Room persistence — detection, 3s qualification and dedup are never bypassed; arbitrary accessibility callbacks are never counted) and increments its own `testCount`. `START TEST` = limit set, count=0, enforced=false, listener registered (idempotent). `RESET TEST` = count 0/enforced false, test state only. `STOP TEST` = mode off + listener unregistered. Nothing is ever persisted.
- **Corrected enforcement seam:** `ShortsRestrictionEngine.debugTriggerEnforcement(testCondition: Boolean)` overrides ONLY the production `limitReached` **decision input** (via a non-persisted `ShortsControlState.copy(limitReached = true)`) so the SAME production `shouldRestrict` decision function still runs, then executes the REAL `ShortsRestrictionOverlayManager.show()` — same overlay, same WindowManager flags, same touch blocking, same permission gate, same `isShowing` guard. Fake decision input in DEBUG is acceptable; fake enforcement action is not. No `FakeRestrictionEngine`/`FakeOverlay`/second engine exists. Production behavior uses the unchanged production decision path. (Note: because the overridden input is never persisted, the next production surface evaluation re-derives the REAL state — correct by design.)
- **HUD integration (mandatory requirement):** the EXISTING floating HUD now shows a clearly identifiable DEBUG indicator — `TEST MODE · n / limit` (plus `· LIMIT` at the crossing) in `colors.Danger` — while test mode is active. It reads the harness state directly (`ShortsHudUiState.testModeActive/testCount/testLimit`, written via Compose snapshot state so the HUD recomposes live on every counted Short); there is no second counter and no second HUD system. With test mode OFF all test fields are false/zero and the production HUD renders exactly as before (additive, DEBUG-only).
- **Production isolation verified by inspection:** `START TEST` / `RESET TEST` / `STOP TEST` make NO Room save, NO production cycle/count/limit/daily reset, NO settings change, NO `debugSetLimit`/`debugSetCount`/`debugResetCycle` use, and never touch `MIN_SHORTS_LIMIT`. The harness holds no reference to any store; the production `limitReached` decision path is untouched.
- **Logging (tag `SHORTS_TEST`):** `TEST_START limit=… count=0` / `TEST_SHORT_ACCEPTED count=N limit=…` / `TEST_LIMIT_REACHED count=… limit=…` / `TEST_ENFORCEMENT_TRIGGERED` / `TEST_ENFORCEMENT_EVAL surfaceActive=… testCondition=… decision=…` / `TEST_ENFORCEMENT_ACTION show success=…` / `TEST_ENFORCEMENT_RESULT result=SHOWN|SHOW_FAILED|DECISION_FALSE|NO_CONTEXT|NOT_DEBUG` / `TEST_RESET` / `TEST_COUNT=0` / `TEST_STOP`. All existing production logs preserved.
- **Negative conditions handled:** count 4/5 → no enforcement; 5/5 → real enforcement once; duplicate callbacks after the crossing → `testEnforced` edge gate prevents re-triggering; RESET → 0/5 and immediately rerunnable (no 24-hour wait); STOP → no further test enforcement; production mode → zero test behavior.
- **Build/tests:** `:app:compileDebugKotlin` PASS; `:app:testDebugUnitTest` 331 tests, 11 failed — the identical pre-existing `ShortsMonitoringPipelineTest` baseline set (no new failures). Physical-device execution of the 5-Short loop: NOT yet performed (implementation phase).

### Phase 4A.2 — Enforcement failure diagnostic instrumentation *(Sep 6, 2026)*

**Files changed:** `shorts/ShortsRestrictionEngine.kt` + `shorts/ShortsEnforcementTestHarness.kt` (DEBUG-only `SHORTS_DIAG` log lines added; **zero behavior changes**), and this `README.md`. Diagnosis phase — no enforcement redesign.

- **What was added (log-only, `BuildConfig.DEBUG`-gated):** a dedicated `SHORTS_DIAG` trace at every enforcement boundary — `TEST_STARTED` / `VALIDATED_SHORT_EVENT testCount=N testLimit=L` / `TEST_LIMIT_REACHED` / `DEBUG_TRIGGER_ENTERED` / `ENFORCEMENT_STATE surfaceActive=… productionLimitReached=… effectiveLimitReached=… productionCount=… productionLimit=…` / `SHOULD_RESTRICT_RESULT result=…` / `OVERLAY_SHOW_REQUEST context=… canDrawOverlays=…` / `OVERLAY_SHOW_ENTER alreadyShowing=…` / `OVERLAY_SKIP reason=ALREADY_SHOWING|NO_OVERLAY_PERMISSION|NO_WINDOW_MANAGER` / `OVERLAY_ATTACHED success=true|false error=…` / `OVERLAY_SHOW_RESULT shown=…` / `SURFACE_EVAL surfaceActive=… productionLimitReached=… decision=… overlayShowingBefore=…` / `COUNT_EVAL …` / `PRODUCTION_HIDE_REQUEST reason=PRODUCTION_EVAL_FALSE` / `OVERLAY_HIDE_REQUEST wasShowing=…` / `TEST_RESET`. The overlay manager's three silent guard exits (already-showing / permission / WindowManager) and the `addView` failure branch are now observable; the production surface/count re-evaluation paths now log their decision and whether they hid a visible overlay.
- **Purpose:** one physical-device 5-Short test now yields an unambiguous trace identifying the exact failing boundary (harness trigger → decision → show() → attach → subsequent production re-evaluation), with every input value that explains the final boolean. Production enforcement behavior, decision semantics, overlay implementation and all guards are untouched — the instrumentation can be retained or removed in 4A.3.
- **Build/tests:** `:app:compileDebugKotlin` PASS; `:app:testDebugUnitTest` 331/11 — identical pre-existing baseline (no new failures).

### Phase 4A.3 — Persistent DEBUG test-enforcement seam *(Sep 6, 2026)*

**Files changed:** `shorts/ShortsEnforcementTestHarness.kt` (+ read-only `isTestEnforced(): Boolean` — `BuildConfig.DEBUG && testModeEnabled && testEnforced`), `shorts/ShortsRestrictionEngine.kt` (+ private `effectiveControlState()` consumed by BOTH production evaluation paths; `debugTriggerEnforcement` one-shot seam kept and its diagnostics extended), and this `README.md`.

- **Root cause fixed (from Phase 4A.2):** the DEBUG seam showed the real overlay at the test-limit crossing, but the next production surface/count evaluation re-read the untouched production state (`limitReached = false`) and hid it — the one-shot test condition was invisible to the stateless re-evaluation paths.
- **Persistent condition:** the engine's two evaluation paths (`onSurfaceStateChanged`, `onCountSignal`) now obtain their state via `effectiveControlState()` — the authoritative production state, except when the DEBUG test is currently enforced, where a **local, non-persisted** `copy(limitReached = true)` is used. The SAME production `shouldRestrict` decision function and the SAME real `ShortsRestrictionOverlayManager` remain in use: no second engine, no second decision, no duplicated logic, no production write, no timer.
- **Read-only dependency:** the engine queries `ShortsEnforcementTestHarness.isTestEnforced()` (read-only; the engine never writes test state). The harness sets `testEnforced = true` BEFORE the one-shot trigger fires (no callback can observe a stale false), and RESET/STOP TEST clear it — after which the next evaluation uses the pure production state and the overlay is hidden through the existing normal path.
- **Lifecycle invariants preserved:** the `testEnforced` edge gate still fires the one-shot trigger exactly once per crossing; START/RESET/STOP semantics unchanged; production isolation unchanged (no Room write, no `debugSetCount`/`debugSetLimit`/`debugResetCycle`, no cycle/count/limit/timestamp mutation).
- **Release safety:** the override is impossible in release builds — `isTestEnforced()` hard-gates on `BuildConfig.DEBUG` and `effectiveControlState()` re-checks it; no configuration flag was introduced.
- **Diagnostics:** `SURFACE_EVAL`/`COUNT_EVAL`/`ENFORCEMENT_STATE` now include `testEnforced=` and `effectiveLimitReached=` so the persistent behavior is verifiable from Logcat; `SHORTS_TEST`/`SHORTS_DIAG` logs otherwise unchanged.
- **Build/tests:** `:app:compileDebugKotlin` PASS; `:app:testDebugUnitTest` 331 tests, 11 failed — identical pre-existing `ShortsMonitoringPipelineTest` baseline (no new failures). Physical-device validation of the persistent loop (5/5 → overlay survives subsequent callbacks → RESET → repeat): NOT yet executed.

### Phase 4A.4 — Test harness execution boundary audit *(Sep 6, 2026)*

**Files changed:** `shorts/ShortsEnforcementTestHarness.kt` + `shorts/ShortsRestrictionEngine.kt` (DEBUG-only boundary logs: `TEST_COUNT_EVENT_RECEIVED` pre-guard, `TEST_COUNT_INCREMENT`, `TEST_ENFORCED_SET`, `DEBUG_TRIGGER_DECISION`, release-build `TEST_START_REJECTED reason=NOT_DEBUG_BUILD`), and this `README.md`. **Diagnostic phase — zero logic changes.**

- **Device evidence under audit:** 5 real `SC_SHORT COUNTED` events with **zero** `SHORTS_TEST`/`SHORTS_DIAG` lines, then `PROCESS ENDED (24730)` / `PROCESS STARTED (27808)` — no overlay shown.
- **Static findings (reported, not fixed):** (1) the harness is event-reachable **only** while test mode is registered via `START TEST`; count events fire regardless (production seam/HUD) but `onValidatedShortEvent()` returns at the `!testModeEnabled` guard — the new pre-guard log makes this observable on device; (2) all harness state (`testModeEnabled`/`testCount`/`testEnforced`) is in-memory and lost on process death — `PROCESS ENDED` resets the test loop and the listener registration (recreated only via the UI's `remember`-scoped objects after restart); (3) `ShortsRestrictionOverlayManager.show()` creates a bare `ComposeView` **without** the `ViewTreeLifecycleOwner`/`SavedStateRegistryOwner` that `ShortsHudOverlayManager` documents as mandatory for overlay ComposeViews — a latent crash source if the seam ever executes `show()`; the HUD overlay sets them explicitly, the restriction overlay does not.
- **Build/tests:** `:app:compileDebugKotlin` PASS; `:app:testDebugUnitTest` 331/11 — identical pre-existing baseline (no new failures).

### Phase 4A.5 — Static execution trace & root-cause audit *(Sep 6, 2026)*

Diagnosis-only phase — **no code changes**. Source audit over the existing Phase 4A.4 audit + supplied device evidence (`5 SC_SHORT COUNTED` with zero `SHORTS_TEST`/`SHORTS_DIAG`, then `PROCESS ENDED (24730)` / `PROCESS STARTED (27808)`, no overlay).

- **Branch B excluded by source:** every path that reaches `ShortsRestrictionOverlayManager.show()` logs synchronously on the same thread *before* the overlay work (harness `TEST_COUNT_EVENT_RECEIVED`/`TEST_SHORT_ACCEPTED`/`TEST_LIMIT_REACHED`; seam `DEBUG_TRIGGER_ENTERED`/`OVERLAY_SHOW_REQUEST`). Five counts with zero such lines prove the enforcement action was never invoked — an overlay crash could only suppress logs emitted after the crash point, not these.
- **Branch A confirmed:** the counting process had no live harness registration — the harness state (`testModeEnabled`/`testCount`/`testLimit`/`testEnforced`) and the pipeline listener list are per-process in-memory, and the observed process restart is the only lifecycle vector that erases an established `START TEST`. Production enforcement also cannot fire at 5 counts (production minimum limit is 50).
- **Latent overlay defect confirmed:** `ShortsRestrictionOverlayManager.show()` builds a bare `ComposeView` for a `TYPE_APPLICATION_OVERLAY` window with no ViewTree lifecycle owners — the exact pre-Phase-21 HUD structure that previously crashed on-device with `ViewTreeLifecycleOwner not found` — a real runtime failure path if the seam is ever reached.
- **Final verdict:** `PASS — ROOT CAUSE CONFIRMED` (test state lost on process restart; overlay crash path latent, not the cause of the observed run).

### Phase 4A.6 — DEBUG harness process-restart safety + restriction overlay lifecycle *(Sep 6, 2026)*

**Files changed:** `shorts/ShortsEnforcementTestHarness.kt` (DEBUG-only SharedPreferences persistence of `testModeEnabled`/`testCount`/`testLimit`/`testEnforced`, process `attach()` restore + automatic idempotent listener re-registration, RESET/STOP now persist a disabled state and unregister), `ShortsCapApplication.kt` (calls `ShortsEnforcementTestHarness.attach(applicationContext)` at startup before monitoring begins), `hud/OverlayLifecycleOwner.kt` (**new** — the HUD's `OverlayLifecycleOwner` extracted into a shared internal component used by both overlays), `hud/ShortsHudOverlayManager.kt` (uses the shared owner — behavior unchanged), `shorts/ShortsRestrictionEngine.kt` (`ShortsRestrictionOverlayManager.show()` now supplies `ViewTreeLifecycleOwner`/`ViewModelStoreOwner`/`SavedStateRegistryOwner` before `setContent`/`addView`; `hide()` disposes the per-show owner; addView log-and-rethrow preserved), `screens/settings/DebugEnforcementPanel.kt` (description updated), `app/src/test/.../ShortsEnforcementTestHarnessTest.kt` (**new** — 13 Robolectric tests), `app/src/test/.../OverlayLifecycleOwnerTest.kt` (**new**), and this `README.md`. Implementation phase — the two Phase 4A.5-confirmed problems.

- **Fix 1 — harness survives process restart:** the four DEBUG test fields are persisted to a dedicated, DEBUG-only SharedPreferences namespace (`shorts_debug_enforcement_test`); `ShortsCapApplication.onCreate` calls `attach()` in DEBUG, which restores the persisted state and — when the restored state has the test enabled — re-registers the count listener with `ShortsMonitoringPipeline.sharedInstance` automatically (no re-pressing START TEST). Registration is idempotent (pipeline dedup + a local guard). `testEnforced=true` is persisted BEFORE the one-shot `debugTriggerEnforcement(true)` trigger (Phase 4A.3 ordering preserved). RESET TEST / STOP TEST persist a disabled state and unregister; the production 24-hour cycle, Room records, `ShortsControlEngine` semantics and `currentState()` are untouched — the harness never references any production store.
- **Fix 2 — restriction overlay lifecycle:** `ShortsRestrictionOverlayManager.show()` now creates one `OverlayLifecycleOwner` per show and sets `ViewTreeLifecycleOwner`/`ViewModelStoreOwner`/`SavedStateRegistryOwner` before `setContent` and `WindowManager.addView` — the same proven mechanism the HUD overlay uses (shared component, no duplicated lifecycle logic). `hide()` disposes the owner (ON_PAUSE/ON_STOP/ON_DESTROY) and clears references so repeated show()/hide() cycles stay clean; genuine `addView` failures remain logged and rethrown (observable).
- **Seam preserved (Phase 4A.3):** `ShortsEnforcementTestHarness.isTestEnforced()` and `ShortsRestrictionEngine.effectiveControlState()` unchanged; both `onSurfaceStateChanged()` and `onCountSignal()` still evaluate via `effectiveControlState()`; the pure `shouldRestrict()` contract unchanged.
- **Release safety:** persistence/restore/registration/panel are all `BuildConfig.DEBUG`-gated; release builds behave exactly as before (no test persistence, no re-registration, `startTest` still rejected).- **Build/tests:** `:app:compileDebugKotlin` PASS; `:app:testDebugUnitTest` **345 tests, 11 failed** — the identical pre-existing `ShortsMonitoringPipelineTest` baseline set; **0 new failures** (14 new tests added and passing: 13 harness + 1 overlay owner).

### Phase 4B — Break restriction-overlay self-dismissal (accessibility feedback loop) *(Sep 6, 2026)*

**Files changed:** `shorts/ShortsMonitoringPipeline.kt` (`onForegroundAppChanged` — own-overlay window events are now ignored as surface exits regardless of session state), and this `README.md`. Implementation phase following the Phase 4A.7 root-cause investigation.

- **Root cause addressed:** the full-screen restriction overlay is a `TYPE_APPLICATION_OVERLAY` ComposeView, and the accessibility service reports its own window as a `com.shortscap.app` / `ComposeView` foreground event. The restriction overlay appears exactly when the limit-crossing count has just reset the Shorts session to `NO_SESSION`, so the pre-existing `isOurOverlay` carry-forward (gated on `sessionInProgress`) did NOT apply: the pipeline treated the overlay as a new non-Shorts surface, broadcast a `null` short-form surface, and `ShortsRestrictionEngine.onSurfaceStateChanged(null)` evaluated `shouldRestrict(false, limitReached=true) == false` and called `hide()` — the overlay removed itself ~1 s after appearing.
- **Fix (broadcast/surface boundary, not `hide()`):** `ShortsMonitoringPipeline.onForegroundAppChanged` now recognises ANY own-overlay foreground event (package `com.shortscap.app` + window class containing `ComposeView` — the same single definition already used by the HUD handling) and returns immediately WITHOUT creating a new surface context and WITHOUT emitting a surface broadcast, regardless of whether a Shorts session is in progress. Own overlays (HUD chip + restriction blocker) are therefore never interpreted as leaving the restricted surface, and no enforcement-clearing `null` broadcast can be generated by the enforcement overlay's own window event.
- **Genuine dismissal preserved:** `ShortsRestrictionEngine` is unchanged. Genuine exits (launcher/other app foreground) still broadcast `null` normally → `onSurfaceStateChanged(null)` → `hide()`; `ShortsRestrictionEngine.stop()` (service shutdown) and RESET/STOP (enforcement inactive → next evaluation hides) are untouched. The overlay is not immortal and enforcement is not weakened.
- **Diagnostics:** `SC_TRACE OWN_OVERLAY_EVENT pkg=… decision=IGNORE_NO_SURFACE_EXIT prevSession=…` at the guard; the existing `SURFACE_EVAL` / `OVERLAY_HIDE_REQUEST` (`SHORTS_DIAG`) lines continue to prove the overlay is preserved (no hide request follows the own-overlay event).
- **Detection semantics unchanged:** qualification, swipe/counting, evidence thresholds, `EVIDENCE_LOST_GRACE_MILLIS`, platform detection and Room persistence untouched. No timers, no repeated-`show()` workaround, no overlay/Activity re-architecture.
- **Build/tests:** `:app:compileDebugKotlin` PASS; `:app:testDebugUnitTest` **345 tests, 11 failed** — the identical pre-existing `ShortsMonitoringPipelineTest` baseline set; **0 new failures** (no test touches the own-overlay event path).
