 ...
 * app, but it also hosts live streams, chats and other long-form content.
 * Detection tiers:
 *
 *  1. Activity class match — short-video activity, confidence 0.80.
 ...
 *  4. Content description text signals — ShareChat-unique labels, confidence 0.60.
...
object ShareChatShortsAdapter : ShortPlatformAdapter {
    ...
    override fun detect(signals: ShortDetectionSignals): ShortDetectionResult {
        ...
        if (className != null && className.lowercase().let {
            ...
            Log.i("SC_PLATFORM_OBS",
                "SC_PLATFORM_OBS ADAPTER_OUTPUT pkg=${signals.packageName} " +
                    "method=ACTIVITY_CLASS surface=SHARE_CHAT_SHORT_VIDEO confidence=0.80"
            )
        ...
        if (result != null) {
            Log.i("SC_PLATFORM_OBS",
                "SC_PLATFORM_OBS ADAPTER_OUTPUT pkg=${signals.packageName} " +
                    "method=CONTENT_EVIDENCE surface=SHARE_CHAT_SHORT_VIDEO confidence=${result.confidence}"
            )
            return result
        }
        ...
        if (textResult != null) {
            Log.i("SC_PLATFORM_OBS",
                "SC_PLATFORM_OBS ADAPTER_OUTPUT pkg=${signals.packageName} " +
                    "method=TEXT_EVIDENCE surface=SHARE_CHAT_SHORT_VIDEO confidence=${textResult.confidence}"
            )
            return textResult
        }
        ...
        if (scrollConf >= ShortFormSurfaceState.CONFIDENCE_THRESHOLD) {
            Log.i("SC_PLATFORM_OBS",
                "SC_PLATFORM_OBS ADAPTER_OUTPUT pkg=${signals.packageName} " +
                    "method=SCROLL_FALLBACK surface=SHARE_CHAT_SHORT_VIDEO confidence=$scrollConf"
            )
            return scrollDetectedResult(ShortPlatform.SHARE_CHAT, ShortSurface.SHARE_CHAT_SHORT_VIDEO, signals.interactionCount)
        }
        ...
        return unconfirmedResult(ShortPlatform.SHARE_CHAT, 0.30f)
    }
    ...
    private fun shortsResult(confidence: Float, surfaceSignal: String): ShortDetectionResult =
        ShortDetectionResult(
            ...
            surface = ShortSurface.SHARE_CHAT_SHORT_VIDEO,
            isShortForm = true,
    ...
    /** Text signals unique to ShareChat short-video. */
    private fun evaluateTextEvidence(evidence: WindowContentEvidence): ShortDetectionResult? {
        val textHit = evidence.nodeContentDescriptions.any { desc ->
            ...
            SHARECHAT_UNIQUE_DESCRIPTIONS.any { lower.contains(it) }
        }
    ...
    /** Content descriptions unique to ShareChat. */
    private val SHARECHAT_UNIQUE_DESCRIPTIONS = setOf(
        "share",
        "like",
        "comment",
        "follow",
